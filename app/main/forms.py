...
class PostForm(FlaskForm):
    ...
    Entry = TextAreaField('Post your article', validators=[
                          DataRequired(), Length(min=1, max=100000000000)])
...
class SubscribersForm(FlaskForm):
    email = StringField('Subscribe to get alerts', validators=[
                        DataRequired(), Email(), Length(min=1, max=200), validate_subscriber])
    submit = SubmitField("Submit")
